...
{
    /// <summary>
    /// definition de la classe UserRepositoryImpl qui implémente l'interface UserRepository,
    ...
    internal class UserRepositoryImpl : UserRepository
    {
        ...
        public UserRepositoryImpl(DbContext dbContext)
        {
        ...
        public void AddUser(User user)
        {
            using var conn = dbContext.CreerConnection();
            conn.Open();
            var command = new MySqlCommand("INSERT INTO users (name, tel, email) VALUES (@name, @tel, @email)", conn);
        ...
        public void UpdateUser(User user)
        {
            using var conn = dbContext.CreerConnection();
            conn.Open();
            var command = new MySqlCommand("UPDATE users SET name = @name, tel = @tel, email = @email WHERE id = @id", conn);
            command.Parameters.AddWithValue("@name", user.name);
            command.Parameters.AddWithValue("@tel", user.tel);
            command.Parameters.AddWithValue("@email", user.email);
